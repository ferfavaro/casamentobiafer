...
const RSVPModal = ({ isOpen, onClose }: RSVPModalProps) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch("https://sua-api.com/confirmar-presenca", {
        method: "POST",
          ...
          >
            ...
            {status === "success" ? (
              <div className="text-center py-4">
                <p className="text-gold text-lg font-display">Presença confirmada!</p>
                <p className="text-muted-foreground text-sm mt-2">Estamos ansiosos para celebrar com você.</p>
                <button
                  onClick={handleClose}
